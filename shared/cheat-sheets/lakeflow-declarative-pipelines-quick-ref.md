---
tags: [cheat-sheet, lakeflow-declarative-pipelines, dlt, data-engineering]
---

# Lakeflow Declarative Pipelines Quick Reference (formerly DLT)

Quick reference for Lakeflow Declarative Pipelines (formerly Delta Live Tables / DLT) — table types, expectations, APPLY CHANGES, and pipeline settings.

> [!important]
> **Current naming:** the product is **Lakeflow Declarative Pipelines** (formerly *Delta Live Tables / DLT*). The Python module is still imported as `dlt`, and `@dlt.table` / `@dlt.expect_or_drop` still work. The newer canonical name is `from pyspark import pipelines as dp` with `@dp.table` and `dp.create_auto_cdc_flow()` — both styles are supported.
...
## Decorator Options (`@dlt.*` — the `dlt` module name is retained for back-compat)
